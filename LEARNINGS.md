...
- A cross-origin preview can become the browser's active Media Session even when the parent page only embeds it. To keep Mac play/pause keys away from Agentic DJ without breaking the player UI, register no-op `play` and `pause` handlers in the top frame and deny the iframe's `mediasession` permission; release the handlers on unmount.
