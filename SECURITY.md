...
Security-sensitive boundaries include localhost binding, same-origin request checks, crate URL/provider validation, the Bandcamp preview relay's track-ID/host/path/size checks, import size limits, revision-safe writes, CSV formula neutralisation, updater origin verification, private-data path protection, and non-destructive recovery.
